...
def main():
    ...
    # pands merge src1 = flights.destination_airport
    # airport.iata to get country
    ...
    src2["domestic"] = pd.NA
    src2["international"] = pd.NA
    for _, row in src2.iterrows():
        domestic = 1
        international = 0
        if row["country_src"] != row["country_dest"]:
            domestic = 0
            international = 1

        row["domestic"] = domestic
